...
const topMobs = computed(() => {
  ...
  activePlayers.value.forEach(p => {
    ds.forEach(date => {
      const pd = sd[date]?.[p] || {}
      Object.entries(pd as Record<string, number>).forEach(([mob, count]) => {
        totals[mob] = (totals[mob] || 0) + count
      })
    })
  })
  return Object.entries(totals).sort((a, b) => b[1] - a[1]).slice(0, 10).map(([key, count]) => ({ key, name: getMobName(key, _l), count }))
})
  ...
  <div class="space-y-6">
    <div class="flex flex-wrap gap-2">
      <button
        ...
        :class="category === 'killed'
          ? 'subnav-active'
          : 'subnav-inactive'"
        @click="category = 'killed'"
      >
        {{ t('battle.killed') }}
      </button>
      <button
        class="px-5 py-2 rounded-lg text-sm font-medium transition-all duration-200"
        :class="category === 'killed_by'
          ? 'subnav-active'
          : 'subnav-inactive'"
        @click="category = 'killed_by'"
      >
        {{ t('battle.killed_by') }}
      </button>
    </div>
      ...
      <div class="relative">
        <div class="flex flex-wrap items-center gap-2 md:gap-4 mb-6">
          <div class="w-12 h-12 bg-gradient-to-br from-brand/20 dark:from-brand/20 to-brand/10 dark:to-brand/15 rounded-xl flex items-center justify-center">
            <Swords class="w-6 h-6 text-brand dark:text-brand-light" />
          </div>
          <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-100">{{ category === 'killed' ? t('battle.killed') : t('battle.killed_by') }}</h3>
        </div>
    ...
    <div class="relative bg-white/70 dark:bg-slate-800/70 backdrop-blur-sm rounded-2xl p-5 md:p-8 border border-white/80 dark:border-slate-700/80 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden group">
      ...
      <div class="relative">
        ...
        <div v-if="topMobs.length === 0" class="flex flex-col items-center justify-center py-16 gap-3">
          ...
          <table class="w-full">
            <thead>
              <tr class="bg-slate-50/80 dark:bg-slate-800/80">
                ...
                <th class="px-4 py-3 text-left text-sm font-medium text-slate-600 dark:text-slate-400">{{ t('common.name') }}</th>
                <th class="px-4 py-3 text-right text-sm font-medium text-slate-600 dark:text-slate-400">{{ t('common.count') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(item, i) in topMobs" :key="item.key" class="border-t border-slate-100 dark:border-slate-700 hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors">
                <td class="px-4 py-3">
                  <span
