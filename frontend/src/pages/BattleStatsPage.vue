  ...
  return {
    ...
    datasets: [
      ...
      })),
      {
        ...
        backgroundColor: 'rgba(255,107,107,0.5)', borderColor: '#FF6B6B',
...
<template>
  <div>
    <div class="stat-tabs">
      <button :class="{ active: category === 'killed' }" @click="category = 'killed'">{{ t('battle.killed') }}</button>
      <button :class="{ active: category === 'killed_by' }" @click="category = 'killed_by'">{{ t('battle.killed_by') }}</button>
    </div>
    <PlayerFilter :filter="filter" />
    <div class="surface-card" style="padding:20px;border-radius:16px;margin-bottom:16px">
      <h3 style="margin-bottom:12px;font-weight:500">{{ category === 'killed' ? t('battle.killed') : t('battle.killed_by') }}</h3>
