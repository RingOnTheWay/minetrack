  ...
  return {
    ...
    datasets: [
      ...
      })),
      {
        ...
        backgroundColor: 'rgba(255,107,107,0.5)', borderColor: '#FF6B6B', borderWidth: 2, hidden: true,
      },
    ],
  }
})
...
<template>
  <div>
    <div class="stat-tabs">
      <button :class="{ active: category === 'crafted' }" @click="category = 'crafted'">{{ t('craft.crafted') }}</button>
      <button :class="{ active: category === 'used' }" @click="category = 'used'">{{ t('craft.used') }}</button>
    </div>
    <PlayerFilter :filter="filter" />
    <div class="surface-card" style="padding:20px;border-radius:16px;margin-bottom:16px">
      <h3 style="margin-bottom:12px;font-weight:500">{{ category === 'crafted' ? t('craft.crafted') : t('craft.used') }}</h3>
