...
class SettingsRepository:
    ...
    def get(key: str) -> str:
        ...
        if row:
            return row['value']
        return SettingsRepository.DEFAULTS.get(key, '')
    ...
    def get_all() -> dict:
        ...
        conn.close()
        result = dict(SettingsRepository.DEFAULTS)
        for row in rows:
            result[row['key']] = row['value']
        return result

    @staticmethod
    def set(key: str, value: str, conn: Optional[sqlite3.Connection] = None):
        close_conn = conn is None
        if close_conn:
            conn = get_connection()
        conn.execute(
    ...
    @staticmethod
    def set_many(settings: dict, conn: Optional[sqlite3.Connection] = None):
        close_conn = conn is None
        if close_conn:
            conn = get_connection()
    ...
    def get_filter_config() -> dict:
        all_settings = SettingsRepository.get_all()
        try:
            min_hours = float(all_settings['min_playtime_hours'])
        except (ValueError, TypeError):
            min_hours = 1.0
        return {
            'filter_enabled': all_settings['filter_enabled'] == 'true',
            'min_playtime_hours': min_hours,
            'whitelist': json.loads(all_settings['whitelist'] or '[]'),
            'blacklist': json.loads(all_settings['blacklist'] or '[]'),
        }