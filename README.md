...
MineTrack 是一个 Vue 3 + Flask 分层架构的 Minecraft 服务器数据统计面板，支持本地数据扫描导入和静态页面部署两种模式。涵盖地图大小趋势、玩家统计、战斗统计、物品合成、物品拾取/丢弃/使用、活动统计等全方位数据可视化。支持暗色模式、9 种主题色切换、中英文双语。
...
pages/            ← Vue 页面组件（按路由懒加载）
    ↓
components/       ← 可复用 UI 组件（ChartContainer / PlayerFilter）
    ↓
